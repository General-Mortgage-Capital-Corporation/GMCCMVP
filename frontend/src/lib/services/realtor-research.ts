/**
 * Core realtor research logic — shared by the API route and the agent tool.
...
export interface RealtorResearchResult {
...
function parseGeminiResponse(data: Record<string, unknown>): AgentResearch | null {
  const parts =
    (data.candidates as { content?: { parts?: { text?: string }[] } }[])?.[0]?.content?.parts ??
    [];
  const rawText = parts
...
function sanitize(r: AgentResearch, hasSources: boolean): AgentResearch {
  return {
    ...
    reviews:
      typeof r.reviews === "string"
        ? r.reviews
        : r.reviews
          ? JSON.stringify(r.reviews)
          : null,
    linkedinSnippet:
      typeof r.linkedinSnippet === "string"
        ? r.linkedinSnippet
        : r.linkedinSnippet
          ? JSON.stringify(r.linkedinSnippet)
          : null,
    personalHooks: Array.isArray(r.personalHooks)
      ? r.personalHooks.map((h) => (typeof h === "string" ? h : JSON.stringify(h)))
      : [],
    sources:
      hasSources && Array.isArray(r.sources)
        ? r.sources.map((s) => (typeof s === "string" ? s : JSON.stringify(s)))
        : [],
...
export async function researchRealtor(
  input: RealtorResearchInput,
): Promise<RealtorResearchResult> {
...
{"summary":"...","specialties":[],"yearsActive":null,"recentActivity":"Unknown","designations":[],"reviews":null,"linkedinSnippet":null,"personalHooks":[],"sources":[],"confidence":"low"}`;
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.log(
        `[realtor-research] Gemini responded in ${Date.now() - t1}ms, parts: [${partTypes.join(" | ")}]`,
      );
      research = parseGeminiResponse(data);
      if (!research) {
        const allText = rawParts
      ...
      console.error(
        `[realtor-research] Gemini search HTTP ${res.status} after ${Date.now() - t1}ms:`,
        errBody.error?.message ?? "no details",
      );
    }
  } catch (err) {
  ...
  // Attempt 2: Without google_search (faster fallback)
  if (!research) {
    const t2 = Date.now();
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.error(
          `[realtor-research] Fallback HTTP ${res.status} after ${Date.now() - t2}ms`,
        );
      }
    } catch (err) {
      console.error(
        `[realtor-research] Fallback exception after ${Date.now() - t2}ms:`,
