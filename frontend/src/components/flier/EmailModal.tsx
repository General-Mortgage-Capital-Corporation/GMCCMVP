...
export default function EmailModal({
  ...
  const generateEmail = useCallback(async (prompt: string, researchOverride?: string | null) => {
    ...
    setError(null);
    try {
      const res = await authedFetch("/api/suggest-email", {
        method: "POST",
  ...
  async function handleSend() {
    ...
    // allowlists a flagged address (after which it verifies as deliverable and
    // sends for everyone). Self-sends skip the check since the LO's own address
    // is presumed valid.
    if (!skipValidation) {
      setVerifying(true);
      try {
